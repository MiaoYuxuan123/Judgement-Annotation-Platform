...
> **对照基准**：`backend/src/main/resources/db/*.sql` + `DatabaseInitializer` 运行时补丁。`deploy/init.sql` 为历史汇总脚本，若与前者不一致，**以前者为准**。
...
| 4a | sys_user | message | 接收 | 1:N | message.user_id → sys_user.id | 任务/裁定等系统通知 |
...
CREATE TABLE IF NOT EXISTS `message` (
    ...
    `type` VARCHAR(20) NOT NULL COMMENT 'TASK / ARBITRATION / SUBMISSION 等',
    `title` VARCHAR(200) NOT NULL COMMENT '消息标题',
    `content` TEXT COMMENT '消息正文',
    `task_id` INT DEFAULT NULL COMMENT '关联任务',
    `task_document_id` INT DEFAULT NULL COMMENT '关联文书（可空）',
    `data_id` INT DEFAULT NULL COMMENT '文书 API ID（用于跳转）',
...
> 说明：由 `TaskService`、`ReviewService` 等在阶段变更、裁定退回等场景写入；超级管理员不展示消息中心 UI，接口返回空列表。
