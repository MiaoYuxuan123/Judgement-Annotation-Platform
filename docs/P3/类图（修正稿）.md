...
> 本文档基于项目当前真实代码与数据库结构编写，对应 `backend/src/main/resources/db/` 与 `backend/src/main/java/edu/nju/jap/` 下的实现。
>
> **持久化实体（共 15 张表）**：sys_user、guide_version、label_l1、label_l2、relation_type、global_document、task、task_member、task_document、**message**、annotation、proposition、argument_relation、relation_member、arbitration_snapshot。
...
| 23 | sys_user | message | 接收 | 1:N | message.user_id → sys_user.id | 系统通知（任务/裁定等） |
...
| 日期 | 变更 |
| --- | --- |
| 2026-06-20 | 补充 `message` 表（15 表）；建表 SQL 对照基准改为 `backend/src/main/resources/db/` |
